...
- mutable_state: the character's CURRENT status that may change later (e.g. "unattached", "wields fire magic", "wounded"). Only NEW or newly-changed current-status items from this turn. A status must be a CONDITION THAT PERSISTS past this moment. Do NOT record a momentary action that completes itself within the passage — "is gathering his thoughts", "is considering his next move", "is walking toward the door", "is speaking in a low voice", "is glancing at the window" are things a person DID this turn, not states they are now in, and recording them makes the story treat a finished gesture as an unfinished one for turns afterward. Ask: if nothing else happened, would this still be true in an hour? If not, leave it out. Do NOT record a physical configuration (a grip, an embrace, a drawn blade, a body position) here at all — those are tracked separately and closed by the action that ends them.
...
- retire_state: CRITICAL for continuity. Copy here, VERBATIM, any item from the character's existing "current state" (shown below) that THIS TURN made false or obsolete. Example: if existing state says "engaged to Lord X" and this turn the engagement is broken, put "engaged to Lord X" in retire_state. A planned meeting, trap, journey, warning, or "about to walk into X" that this turn already carried out or returned from is obsolete — retire it. Leave [] if nothing became false. NEVER let an outdated status linger.
