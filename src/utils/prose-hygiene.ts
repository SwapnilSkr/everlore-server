...
function looksIncompleteEnding(text: string): boolean {
  ...
  if (
    /\b(?:and|but|because|then|as|while|when|if|though|although|so|or|with|without|to|from|into|toward|towards|before|after|until|unless)$/i.test(
      end,
    )
  ) {
    return true;
...
function plainSegmentLooksLikeNarration(
  segment: string,
  names: string[],
): boolean {
  ...
  const actionVerb =
    /\b(?:lets?|let|turns?|tilts?|cocks?|steps?|moves?|leans?|looks?|watches?|smiles?|laughs?|frowns?|pauses?|hesitates?|reaches?|takes?|sets?|drops?|raises?|lowers?|flickers?|glows?|crackles?|burns?|dances?|hangs?|answers?|tightens?|softens?|shifts?|draws?|breathes?|whispers?|murmurs?|says?)\b/i;
  const narrationNoun =
    /\b(?:silence|hearth|fire|flame|flames|embers?|room|air|shadow|eyes?|voice|tone|smile|hand|hands|palms?|pause|moment|warmth|light|darkness)\b/i;
  const nameStart = names.some((name) =>
    new RegExp(`^${escapeRegExp(name)}\\b`, "i").test(compact),
  );
  ...
  return sentences.some((sentence) => {
    ...
    return (
      looksLikeAction &&
      (actionVerb.test(sentence) || narrationNoun.test(sentence))
    );
  });
}
...
function issueScore(issues: ProseHygieneIssue[]): number {
  return issues.reduce((sum, issue) => {
    ...
    if (issue.code === "player_label_leak") return sum + 4;
...
export function validateProseHygiene(
  input: ProseHygieneInput,
): ProseHygieneIssue[] {
  ...
  if (
    /^\s*```/m.test(trimmed) ||
    /^\s*(narrative|state_mutations|flag_mutations|scene_tag|emotional_tone)\s*:/im.test(
      trimmed,
    )
  ) {
    issues.push({
      ...
      severity: "error",
      message:
        "Response appears to contain code fences or structured metadata.",
    });
  }
  ...
  if (/\*\*[^*]+?\*\*/.test(trimmed)) {
    issues.push({
      ...
      severity: "warning",
      message:
        "Response uses double-asterisk narration markers; use single *...* markers.",
    });
  }
  ...
  if (looksIncompleteEnding(trimmed)) {
    issues.push({
      ...
      severity: "warning",
      message:
        "Response appears to end mid-sentence or without a clean terminal beat.",
    });
  }

  const leakedPlayerLabel = playerLabelLeak(trimmed, input.playerAddressMode);
  if (leakedPlayerLabel) {
    issues.push({
    ...
    if (requestedLength === "short") {
      ...
      if (paragraphs < 2 && words >= 100) {
        issues.push({
          ...
          severity: "warning",
          message:
            "Medium reply should be shaped into a few short paragraphs, not one dense block.",
          detail: `${words} words, ${paragraphs} paragraphs`,
        });
      }
      if (paragraphs > 4) {
  ...
  const unquotedSegment = plainSegmentsOutsideNarration(trimmed).find(
    (segment) => /[A-Za-z]/.test(unquotedTextOutsideNarration(segment)),
  );
  if (unquotedSegment) {
    issues.push({
      ...
      severity: "warning",
      message:
        "Response has text outside *...* markers that is not wrapped as quoted speech.",
      detail: unquotedSegment.slice(0, 180),
    });
  }

  const unmarkedNarration = plainSegmentsOutsideNarration(trimmed).find(
    (segment) => plainSegmentLooksLikeNarration(segment, names),
  );
  if (unmarkedNarration) {
    issues.push({
      ...
      severity: "warning",
      message:
        "Response appears to leave narration/action text outside *...* markers.",
      detail: unmarkedNarration.slice(0, 180),
    });
  }

  if (names.length > 0) {
    const previousOpeningNames = normalizedNames(
      input.previousOpeningNames || [],
    );
    const repeatedOpening = previousOpeningNames.find((name) =>
      startsWithName(trimmed, name),
    );
    if (repeatedOpening) {
      issues.push({
        ...
        severity: "warning",
        message:
          "Response opens with the same character name as the previous turn.",
        detail: repeatedOpening,
      });
    }

    const avoidOpeningNames = normalizedNames(input.avoidOpeningNames || []);
    const avoidedOpening = avoidOpeningNames.find((name) =>
      startsWithName(trimmed, name),
    );
    if (avoidedOpening) {
      issues.push({
        ...
        severity: "warning",
        message:
          "Response opens with a character name that should be varied for this turn.",
    ...
    if (anyNameOpening && !repeatedOpening && !avoidedOpening) {
      issues.push({
        ...
        severity: "warning",
        message:
          "Response opens with a known character name instead of a natural beat.",
      ...
      for (let i = 1; i < starts.length; i++) {
        if (startRe.test(starts[i - 1]) && startRe.test(starts[i])) {
          issues.push({
            ...
            severity: "warning",
            message:
              "Consecutive sentences start with the same character name.",
            detail: name,
          });
          i = starts.length;
          break;
      ...
      if (shortName && shortName.toLowerCase() !== name.toLowerCase()) {
        ...
        if (shortMentionCount >= 3) {
          issues.push({
            ...
            severity: "warning",
            message:
              "Response repeats a short character name too often for one turn.",
    ...
    if (mentioned.length >= 4) {
      issues.push({
        ...
        severity: "warning",
        message:
          "Response mentions many codex character names, which may indicate codex leakage.",
...
export async function repairProseHygiene(
  ...
  const initialIssues = validateProseHygiene({
    ...
    playerAddressMode: input.playerAddressMode,
  ...
  if (!initialIssues.some((issue) => REPAIRABLE_CODES.has(issue.code))) {
    return {
      ...
      repaired: normalized !== input.narrative.trim(),
    };
  }
...
Player address mode: ${input.playerAddressMode || "you"}
  ...
  try {
    ...
    if (repaired && issueScore(repairedIssues) < issueScore(initialIssues)) {
      return {
        ...
        repaired: repaired !== input.narrative.trim(),
      };
    }
  } catch {
